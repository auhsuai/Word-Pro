...
def duong_dan_luu_tru(ten_file):
    """Trả về đường dẫn ghi được cho dữ liệu người dùng (tránh thư mục read-only)"""
    thu_muc = os.path.join(os.environ.get('APPDATA', os.path.expanduser('~')), 'WordPro')
    os.makedirs(thu_muc, exist_ok=True)
    return os.path.join(thu_muc, ten_file)

# Đường dẫn file lưu câu tùy chỉnh (cần ghi được → dùng APPDATA)
CUSTOM_SENTENCES_FILE = duong_dan_luu_tru("custom_sentences.json")
# Đường dẫn file từ vựng bổ sung (chỉ đọc, đi kèm app)
...
# --- Persistence & Stats ---
def get_stats_file():
    # Lưu trong APPDATA để tránh lỗi quyền ghi ở Program Files
    return duong_dan_luu_tru("user_stats.json")
